...
export function buildEbayGradedSearchUrl(name: string, sport: string): string {
  const query = encodeURIComponent(`${name} ${sport}`);
  const sportParam = encodeURIComponent(sport);
  const grader = encodeURIComponent("Professional Grader") + "=" + encodeURIComponent("Professional Sports Authenticator (PSA)");
  return `https://www.ebay.com/sch/i.html?_nkw=${query}&_sacat=261328&LH_BIN=1&LH_PrefLoc=2&mkevt=1&mkcid=1&mkrid=706-53473-19255-0&campid=5339142305&toolid=10001&Sport=${sportParam}&Graded=Yes&${grader}&_dcat=261328`;
}
